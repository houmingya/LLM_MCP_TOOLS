...
**基于 FastMCP 2.0 的智能工具调度系统 | 支持知识图谱、向量搜索、实时进度显示**

[特性](#-核心特性) • [快速开始](#-快速开始) • [架构](#-系统架构) • [文档](#-文档) • [贡献](#-参与贡献)
...
| 类别 | 工具数量 | 主要功能 |
|------|---------|---------|
| 📊 **数据库工具** | 6个 | 员工查询、部门统计、薪资分析 |
| 📚 **知识库工具** | 3个 | 文档上传、向量搜索、文档管理 |
| 🧠 **知识图谱工具** | 7个 | 实体抽取、关系构建、图谱查询、路径分析 |
| 🧮 **计算工具** | 3个 | 数学运算、统计分析、单位转换 |
| ⏰ **时间工具** | 4个 | 当前时间、日期计算、时区转换 |
| 🌐 **API工具** | 3个 | HTTP请求、天气查询、外部接口 |
...
重启 MCP Server，Web App 会**自动识别新工具**！✨  
无需修改 `main.py`、工具列表或前端代码。

### 工具开发最佳实践

1. **清晰的文档字符串**：会作为工具描述传递给大模型
2. **类型注解**：帮助自动生成工具参数schema
3. **统一的返回格式**：建议包含 `success`、`data`、`message` 字段
...
- [知识图谱技术深度解析](docs/知识图谱技术深度解析.md)
- [知识图谱应用说明](docs/知识图谱应用说明.md)
- [文本块处理进度功能说明](docs/文本块处理进度功能说明.md)
- [MCP API完整对照表](docs/MCP_API_完整对照表.md)
...
- [FastMCP](https://gofastmcp.com/) - 优秀的 MCP 实现框架
...
- Issues: [项目问题反馈](https://github.com/houmingya/LLM_MCP_TOOLS/issues)
